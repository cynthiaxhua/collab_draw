...
def encode(input_strokes):
	strokes = to_big_strokes(input_strokes,max_len=189).tolist()
...
class MyPaintWidget(Widget):
	...
	def on_touch_down(self, touch):
		...
		if touch.y < 600 and touch.x > 0:
			with self.canvas:
				...
				#bg_color = self.bg_array[int(touch.x),int(touch.y)]
				#print(bg_color)
	...
	def machine_draw(self,m_stroke,x0,y0):
		...
		for point in m_stroke:
			...
			with self.canvas:
				Color(self.r / 255.0, self.g / 255.0, self.b / 255.0, self.t / 255.0)
				Line(points=xy_line,width=self.pen_width, joint = 'bevel',cap = 'square')
				xy_line = xy_line[-2:]
			'''
		...
		with self.canvas:
			Color(self.r / 255.0, self.g / 255.0, self.b / 255.0, self.t / 255.0)
			#bg_color = self.bg_array[int(x0),int(y0)]
			#Color(bg_color[0] - 10 ,bg_color[1] - 10,bg_color[2] - 10, cap = 'square')
			Line(points=xy_line,width = self.pen_width, joint = 'bevel',cap = 'square')
		Window.screenshot(name="screenshots/"+str(self.save_frame_counter)+'.png')
		self.save_frame_counter += 1
